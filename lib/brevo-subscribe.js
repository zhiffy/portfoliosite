...
async function verifyRecaptcha(token, secretKey) {
  ...
  if (!token) return { skipped: true, ok: true, reason: "missing token" };
...
export async function subscribeToStudioUpdates(payload = {}, env = process.env) {
  ...
  // Treat reCAPTCHA as a soft signal only. Privacy tools, slow script loads, or
  // occasional score mismatches should not block real newsletter subscribers.
  await verifyRecaptcha(cleanString(payload.recaptchaToken), recaptchaSecret);
